#' @title Parallel Multiple Factor Analysis (parMFA)
...
parMFA <- function (base, group, type = rep("s", length(group)), ind.sup = NULL, 
                  ncp = 5, name.group = NULL, num.group.sup = NULL, graph = TRUE, 
                  weight.col.mfa = NULL, row.w = NULL, axes = c(1, 2), tab.comp = NULL, cores) 
{
  
  # THIS piece of code was added as when there are duplicate factors in a column, the reweighting of variables in a later change is wrong due to the order changing when they decide to rename the variables for you
  
  base <- as.data.frame(base)
  check_class <- which(sapply(base, class) %in% c("factor", "character"))
  for(i in check_class) 
  {
    base[ ,i] <- factor(base[ ,i], exclude = NULL)
    levels(base[ ,i]) <- paste0(levels(base[,i]))
  }
  
  factor_checks <- lapply(base,  levels)
  
  dup_fac <- which(duplicated(factor_checks) & !sapply(factor_checks, is.null))
  if(length(dup_fac) != 0)
  {
    for(i in 1:length(dup_fac)){
    levels(base[,dup_fac[i]]) <- paste(levels(base[,dup_fac[i]]), i, sep="_")
    }
  }
  ...
  for (i in 1:length(group)) {
    ...
    if (((type[i] == "s") || (type[i] == "c")) && (!(i %in% num.group.sup))) 
      type.var <- c(type.var, rep("quanti", group[i]))
    
    if (((type[i] == "s") || (type[i] == "c")) && (i %in% num.group.sup)) 
      type.var <- c(type.var, rep("quanti.sup", group[i]))
    
    if (((type[i] == "f") || (type[i] == "f2") || (type[i] == "f3")) && (!(i %in% num.group.sup))) 
      type.var <- c(type.var, rep(type[i], group[i]))
    
    if (((type[i] == "f") || (type[i] == "f2") || (type[i] == "f3")) && (i %in% num.group.sup)) 
      type.var <- c(type.var, rep(paste(type[i], "sup", sep = "_"), group[i]))
  }
  ...
  # colnames(base) <- paste0("V", 1:ncol(base))
  # rownames(base) <- paste0("row", 1:nrow(base))
  # for (j in 1:ncol(base)) if (colnames(base)[j] == "") 
    # colnames(base)[j] = paste("V", j, sep = "")
  # for (j in 1:nrow(base)) if (is.null(rownames(base)[j])) 
    # rownames(base)[j] = paste("row", j, sep = "")
  ########################################################
  ...
  if (any(is.na(base))) {
    if (!("n" %in% type)){ 
      for (j in 1:ncol(base)) base[, j] <- replace(base[, j], is.na(base[, j]), mean(base[, j], na.rm = TRUE))
    } else {
        if (type[1] != "n") 
          for (j in 1:group[1]) base[, j] <- replace(base[, j], is.na(base[, j]), mean(base[, j], na.rm = TRUE))
          
          for (g in 2:nbre.group) {
            if (type[g] != "n") 
              for (j in (sum(group[1:(g - 1)]) + 1):sum(group[1:g])) base[, j] <- replace(base[, j], is.na(base[, j]), mean(base[, j], na.rm = TRUE))
          }
          if (is.null(tab.comp)) {
            if (type[1] == "n") 
              for (j in 1:group[1]) base[, j] <- 
                  as.factor(replace(as.character(base[, j]), is.na(base[, j]), paste(colnames(base)[j], ".NA", sep = "")))
              
              for (g in 2:nbre.group) {
                if (type[g] == "n") 
                  for (j in (sum(group[1:(g - 1)]) + 1):sum(group[1:g])) base[, j] <- 
                      as.factor(replace(as.character(base[, j]), is.na(base[, j]), paste(colnames(base)[j], ".NA", sep = "")))
              }
          }
      }
  ...
  if (any("f" %in% type) + any("f2" %in% type) + any("f3" %in% type) > 1) 
  ...
  if (("f" %in% type) || ("f2" %in% type) || ("f3" %in% type)) {
    ...
    for (i in grfrec) {
      if ((type[i] == "f2") || (type[i] == "f3") || (i %in% num.group.sup)) {
        ...
        else base[, (sum(group[1:(i - 1)]) + 1):sum(group[1:i])] <- 
            base[, (sum(group[1:(i - 1)]) + 1):sum(group[1:i])]/sum(base[1:nb.actif, (sum(group[1:(i - 1)]) + 1):sum(group[1:i])])
      }
    }
    type.var == "f"
    if (!any(type.var == "f")) 
    ...
    base[, as.logical((type.var == "f") + (type.var == "f_sup") + (type.var == "f2") + (type.var == "f2_sup") + (type.var == "f3") + (type.var == "f3_sup"))] <- 
      base[, as.logical((type.var =="f") + (type.var == "f_sup") + (type.var == "f2") + (type.var == "f2_sup") + (type.var == "f3") + (type.var == "f3_sup"))]/sumT
  ...
  for (g in 1:nbre.group) {
    ...
    if (type[g] == "s") 
      res.separe[[g]] <- PCA(aux.base, ind.sup = ind.sup, 
                             ...
                             graph = FALSE, col.w = weight.col.mfa[(ind.grpe + 1):(ind.grpe + group[g])])
    if (type[g] == "c") 
      res.separe[[g]] <- PCA(aux.base, ind.sup = ind.sup, scale.unit = FALSE, ncp = ncp, row.w = row.w, 
                             graph = FALSE, col.w = weight.col.mfa[(ind.grpe + 1):(ind.grpe + group[g])])
    
    if (type[g] == "f" || type[g] == "f2" || type[g] == "f3") 
      res.separe[[g]] <- PCA(aux.base, ind.sup = ind.sup, 
                             scale.unit = FALSE, ncp = ncp, row.w = row.w, 
                             graph = FALSE, col.w = F.jt[[g]] * weight.col.mfa[(ind.grpe + 1):(ind.grpe + group[g])])
    if (type[g] == "n") {
      ...
      res.separe[[g]] <- MCA(aux.base, ind.sup = ind.sup, 
                             ncp = ncp, graph = FALSE, row.w = row.w)
    }
    if (!is.null(tab.comp)) {
      if (type[g] == "s") 
        res.separe[[g]] <- PCA(tab.comp[, ind.var.group[[g]]], 
                               scale.unit = TRUE, row.w = row.w, ind.sup = ind.sup, 
                               col.w = weight.col.mfa[(ind.grpe + 1):(ind.grpe + group[g])], graph = FALSE)
      ...
      if (type[g] == "n") 
        res.separe[[g]] <- MCA(aux.base, ind.sup = ind.sup, 
                               ncp = ncp, graph = FALSE, row.w = row.w, tab.disj = tab.comp[, ind.var.group[[g]]])
    }
  ...
  for (g in 1:nbre.group) {
    ...
    if (type[g] == "n") {
      ...
      if (!is.null(tab.comp)) 
        ecart.type.tmp <- sqrt(centre.tmp * sum(row.w.moy.ec) * (1 - centre.tmp * sum(row.w.moy.ec)))/sum(row.w.moy.ec)
    ...
    for (i in 1:nbre.group) {
      if (i %in% num.group.sup) {
        if ((type[i] == "c") || (type[i] == "f")) 
          supp.quanti = c(supp.quanti, (1 + nb.of.var):(nb.of.var + group.mod[i]))
        
        if (type[i] == "n") 
          supp.quali = c(supp.quali, (1 + nb.of.var):(nb.of.var + group.mod[i]))
        ...
        if (ncol(data.group.sup) > 1) 
          colnames.data.group.sup <- c(colnames.data.group.sup, colnames(data)[(1 + nb.of.var):(nb.of.var + group.mod[i])])
  ...
  for (g in 1:nbre.group) {
    if (type[g] == "n") 
    ind.quali <- c(ind.quali, c((ind.var + 1):(ind.var + group[g])))
  ...
  if (!is.null(ind.quali)) {
    ...
    if (is.null(data.group.sup)) 
      aux.quali.sup.indice <- (ncol(data) + 1):(ncol(data) + ncol(aux.quali.sup))
  ...
  if ((!is.null(tab.comp)) & (any("n" %in% type))) {
    ...
    res.globale$call$quali.sup$barycentre <- sweep(crossprod(tab.comp[, unlist(ind.var.group[type %in% "n"])], 
                                                             as.matrix(data.pca)), 1, apply(tab.comp[, unlist(ind.var.group[type %in% "n"])], 2, sum), FUN = "/")
    res.globale$quali.sup$coord <- sweep(crossprod(tab.comp[, unlist(ind.var.group[type %in% "n"])], 
                                                   res.globale$ind$coord), 1, apply(tab.comp[, unlist(ind.var.group[type %in% "n"])], 2, sum), FUN = "/")
  }
  ...
  for (g in 1:length(group.actif)) {
    if (group.mod[group.actif[g]] != 1) 
      contrib.group[g, ] <- apply(res.globale$var$contrib[(ind.var + 1):(ind.var + group.mod[group.actif[g]]), ]/100, 2, sum)
    
    else contrib.group[g, ] <- res.globale$var$contrib[ind.var + 1, ]/100
    
    ind.var <- ind.var + group.mod[group.actif[g]]
    
    dist2.group[g] <- sum((res.separe[[group.actif[g]]]$eig[1:min(ncol(contrib.group), nrow(res.separe[[group.actif[g]]]$eig)), 1]/
                             res.separe[[group.actif[g]]]$eig[1, 1])^2)
  }
  ...
  if (!is.null(num.group.sup)) {
    ...
    if (is.null(ind.sup))
      tab <- (cov.wt(cbind.data.frame(res.globale$ind$coord, data.group.sup), wt = row.w/sum(row.w), method = "ML")$cov)^2
    ...
    for (gc in 1:length(num.group.sup)) {
      if (group.mod[num.group.sup[gc]] != 1) 
        coord.group.sup[gc, ] <- apply(tab[1:ncp.tmp, (ncp.tmp + ind.gc + 1):(ncp.tmp + ind.gc + group.mod[num.group.sup[gc]])], 1, sum)
  ...
  if (is.null(num.group.sup)) {
    ...
    if (is.null(ind.sup))
    {
      tab <- (cov.wt(cbind.data.frame(data, data.group.sup), wt = row.w/sum(row.w), method = "ML")$cov)^2
    }else{
      tab <- (cov.wt(cbind.data.frame(data[-ind.sup, ], data.group.sup[-ind.sup, ]), wt = row.w/sum(row.w), method = "ML")$cov)^2
    }
  ...
  for (f in 1:ncp.tmp) {
    for (g in 1:length(group.actif))
      {
      cor.grpe.fact[g, f] <- cov.wt(cbind.data.frame(res.ind.partiel[[group.actif[g]]]$coord.sup[1:nb.actif, f],
                                                     res.globale$ind$coord[, f]),
                                    wt = row.w/sum(row.w),
  ...
  cor.grpe.fact_l <- foreach (f = 1:ncp.tmp) %dopar% {
    cor.grpe.fact_inner <-list()
    for (g in 1:length(group.actif))
      {
      cor.grpe.fact_inner[[g]] <- cov.wt(cbind.data.frame(res.ind.partiel[[group.actif[g]]]$coord.sup[1:nb.actif, f],
                                                     res.globale$ind$coord[, f]),
                                    wt = row.w/sum(row.w),
                                    method = "ML", cor = TRUE)$cor[1, 2]
    }
    do.call(rbind, cor.grpe.fact_inner)
  }
  
  cor.grpe.fact <- data.frame(do.call(cbind, cor.grpe.fact_l))
  colnames(cor.grpe.fact) <- paste("Dim", c(1:ncp.tmp), sep = ".")
  rownames(cor.grpe.fact) <- name.group[group.actif]
  }
  ...
  for (g in group.actif)
  {
    It <- It + apply(res.ind.partiel[[g]]$coord.sup[1:nb.actif,]^2 * row.w, 2, sum)
  }
  rap.inertie <- apply(res.globale$ind$coord^2 * row.w, 2, sum) * length(group.actif)/It
  res.groupes <- list(Lg = Lg, 
                      RV = RV, 
                      coord = coord.group[,1:ncp], 
                      contrib = contrib.group[, 1:ncp] * 100, 
                      cos2 = cos2.group[,1:ncp], 
                      dist2 = dist2.group[-length(dist2.group)], 
                      correlation = cor.grpe.fact[, 1:ncp])
  ...
  # inertie.intra.ind.partiel <- as.data.frame(matrix(NA, (nb.actif * 
  #                                                          length(group.actif)), ncp.tmp))
  nom.ligne <- NULL
  # for (i in 1:nb.actif) nom.ligne <- c(nom.ligne, paste(rownames(base)[i],
  #                                                       name.group[group.actif], sep = "."))
  # for (i in 1:length(group.actif))
  # {
  #   nom.ligne <- c(nom.ligne, paste(rownames(base)[1:nb.actif], name.group[group.actif[i]], sep = "."))
  # }
  
  nom.ligne <- c(t(outer(rownames(base)[1:nb.actif], name.group[group.actif], paste, sep=".")))
  
  tmp <- array(0, dim = c(dim(res.globale$ind$coord), length(group.actif)))
  for (g in 1:length(group.actif)) 
    tmp[, , g] <- (res.ind.partiel[[group.actif[g]]]$coord.sup[1:nb.actif, ] - res.globale$ind$coord)^2/length(group.actif)
  ...
  #   Section 19 --------------------------------------------------------
  # Inserted list here in the loop, instead of assigning to data.frame (hope this helps when dataframe is large)
  if(missing(cores)){
    inertie.intra.ind.partiel <- list()
    for (i in 1:nb.actif){
      inertie.intra.ind.partiel[[i]] <- t(tmp[i, 1:ncp.tmp, ])
    }
    inertie.intra.ind.partiel <- do.call(rbind, inertie.intra.ind.partiel)
  }else{
    inertie.intra.ind.partiel <- foreach (i = 1:nb.actif) %dopar% {
      t(tmp[i, 1:ncp.tmp, ])
    }
    stopCluster(cl)
    inertie.intra.ind.partiel <- do.call(rbind, inertie.intra.ind.partiel)
  ...
  #   Section 20
    # Inserted list here in the loop, instead of assigning to data.frame (hope this helps when dataframe is large)
  tab.partial.axes <- list()
  
  for (g in 1:nbre.group) {
    nom.col <- paste(nom.axes, name.group[g],sep = ".")
    nbcol <- min(ncp, ncol(res.separe[[g]]$ind$coord))
    x <- res.separe[[g]]$ind$coord[,1:nbcol]
    colnames(x) <- nom.col[1:nbcol]
    tab.partial.axes[[g]] <- x
    debut <- debut + ncp
  }
  
  tab.partial.axes <- do.call(cbind, tab.partial.axes)
  # for (g in 1:nbre.group) {
  ...
  for (g in 1:nbre.group) {
    if ((type[g] == "c") || (type[g] == "f")) {
      ...
      if (!is.null(res.separe[[g]]$call$ecart.type)) 
        statg[, -c(1, 2)] <- round(statg[, -c(1, 2)], digits = 2)
  ...
  for (g in 1:length(group.actif)){
    coord.ind.partiel[liste.ligne + g - 1, ] <- res.ind.partiel[[group.actif[g]]]$coord.sup[, 1:ncp, drop = FALSE]
  }
  ...
  if (!is.null(ind.sup)) {
   ...
   res.ind <- list(coord = coord.ind, contrib = contrib.ind, 
  ...
  if (!is.null(ind.quali)) {
    ...
    if (!is.null(commun)) 
      contrib.quali[commun, ] <- res.globale$var$contrib[commun, 1:ncp, drop = FALSE]
    ...
    for (g in 1:length(group.actif)) {
      ...
      cg.partiel[, (ind.col + 1):(ind.col + group.mod[group.actif[g]])] <- 
        barycentre[, (ind.col + 1):(ind.col + group.mod[group.actif[g]])]
    ...
    for (g in 1:nbre.group) {
      if (type[g] == "n") {
        if (g %in% num.group.sup) 
          ind.col.sup <- c(ind.col.sup, (ind.col + 1):(ind.col + group.mod[g]))
    ...
    if (!is.null(ind.col.sup)) {
      ...
      coord.quali.partiel.sup <- coord.quali.partiel[unlist(lapply(ind.col.sup, 
                                                                   function(k) seq(length(group.actif) * (k - 1) + 1, length = length(group.actif)))), ]
      ...
      inertie.intra.cg.partiel.sup <- inertie.intra.cg.partiel[unlist(lapply(ind.col.sup, 
                                                                             function(k) seq(length(group.actif) * (k - 1) + 1, length = length(group.actif)))), 1:ncp]
      bool.sup <- TRUE
    }
  ...
  for (g in group.actif) {
    ...
    if (type[g] == "f") 
      indice.freq <- c(indice.freq, c((num.tmp + 1):(num.tmp + group.mod[g])))
  ...
  res.partial.axes <- list(coord = coord.res.partial.axes[, 1:ncp], cor = cor.res.partial.axes[, 1:ncp], 
                           contrib = contrib.res.partial.axes[, 1:ncp], cor.between = cor.partial.axes)
  ...
  class(resultats) <- c("MFA", "list")
    ...
    plot.MFA(resultats, choix = "ind", invisible = "quali", 
             partial = rownames(resultats$ind$coord)[max.inertia[c(1:2, nrow(resultats$ind$coord) - 1, nrow(resultats$ind$coord))]], 
